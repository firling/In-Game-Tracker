...
class ChampionDataService {
  private championCache: Map<number, string> = new Map();
  ...
  async initialize() {
    ...
    try {
      ...
      // Build cache: championId -> championName
      for (const championKey in champions) {
        const champion = champions[championKey];
        this.championCache.set(parseInt(champion.key), champion.name);
      }

      this.cacheInitialized = true;
      console.log(`✅ Champion data loaded: ${this.championCache.size} champions`);
    } catch (error) {
      console.error('Error loading champion data:', error);
      // Fallback: use a basic mapping for most common champions
      this.loadFallbackData();
    }
  }

  getChampionName(championId: number): string {
    return this.championCache.get(championId) || `Champion ${championId}`;
  }

  private loadFallbackData() {
    // Basic fallback for common champions
    const fallback: { [key: number]: string } = {
      ...
      895: 'Nilah', 897: 'K\'Sante', 902: 'Milio', 950: 'Naafiri', 901: 'Smolder'
    };

    for (const [id, name] of Object.entries(fallback)) {
      this.championCache.set(parseInt(id), name);
    }
    
    this.cacheInitialized = true;
    console.log('⚠️  Using fallback champion data');
  }
}

export const championData = new ChampionDataService();